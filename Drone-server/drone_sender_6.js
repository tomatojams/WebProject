...
const sendDronePosition = async (droneId, initialLat, initialLng, droneName) => {
  try {
    ...
    setInterval(() => {
      ...
      const position = {
        message_type: "Update Info", // 새로운 메시지 타입
...
const init = async () => {
  ...
  sendDronePosition("a7:b8:c9:d0:e1:f2", 37.567, 126.9788, "DJI Spark"); // 추가된 드론
  sendDronePosition("b3:c4:d5:e6:f7:g8", 37.5671, 126.979, "DJI Matrice"); // 추가된 드론
