...
wss.on("connection", (backSocket) => {
  // 자바스크립트의 동적 속성 추가
  backSocket.username = "미정";
  socketList.push(backSocket);

  console.log(" connection 이벤트 발생:연결신청이 왔음");
  ...
  backSocket.on("message", (message) => {
    ...
    try {
      ...
      msg = { type: "message", text: message.toString() };
    }

    if (msg.type === "username") {
      backSocket.username = msg.name;
      backSocket.send("이름이 입력되었습니다.");
      console.log("새로운 이름:", msg.name);
    } else if (msg.type === "message") {
      console.log(`${backSocket.username}에게서 도착한 메세지:`, msg.text);
      socketList.forEach((asocket) => {
        // if (asocket !== backSocket) {
        // 보낸 본인을 제외하고 나머지에게 보냄
        asocket.send(`${backSocket.username}:${msg.text}`);
        // }
      });
    }
  });
