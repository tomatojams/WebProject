...
function logToFile(content) {
  const logFilePath = path.join(__dirname, 'drone_log.txt');
...
// 1분마다 버퍼의 최신 데이터와 클라이언트의 최근 요청을 로그 파일에 기록
setInterval(() => {
  const latestDroneMessage = messageBuffer[0]; // 버퍼에서 가장 최신의 드론 메시지
  if (latestDroneMessage) {
    logToFile(`Latest Drone Message: ${JSON.stringify(latestDroneMessage)}`);
  } else {
    logToFile("No drone messages in buffer.");
  }

  if (lastClientRequest) {
    logToFile(`Latest Client Request: ${JSON.stringify(lastClientRequest)}`);
  } else {
    logToFile("No client requests have been made yet.");
  }
}, 60000); // 60000 밀리초 = 1분
